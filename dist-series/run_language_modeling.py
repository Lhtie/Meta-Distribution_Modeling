...
import sys
sys.path.append("..")
import modeling.GenTrainer as GenTrainer
...
def get_dataset(args: DataTrainingArguments, tokenizer: PreTrainedTokenizer, evaluate=False):
    ...
    if args.line_by_line:
        ...
        #         for abstract in pack["abstracts"]:
        #             dataset["prompts"].append(pack["article"] + "\n\nThe article can be summarized as:")
        #             dataset["text"].append(pack["article"] + "\n\nThe article can be summarized as: " + abstract + tokenizer.eos_token)
        assert len(inputs) == len(outputs)
        for i, o in zip(inputs, outputs):
            dataset["text"].append(i + " = " + o + tokenizer.eos_token)
            dataset["prompts"].append(i + " =")
    else:
    ...
    def tokenize(examples):
        ...
        model_inputs["weight_mask"] = torch.stack([
            torch.concat([torch.tensor([0.1] * len(tokenizer.encode(p))), 
                          torch.tensor([1.0] * (args.block_size - len(tokenizer.encode(p))))])
            for p in examples["prompts"]
        ])
        return model_inputs
...
def main():
    ...
    if data_args.eval_data_input_file is None and training_args.do_eval:
        raise ValueError(
    ...
    if data_args.block_size <= 0:
        ...
        data_args.block_size = min(data_args.block_size, 512)
